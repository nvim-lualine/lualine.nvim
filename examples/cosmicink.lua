...
local function create_separator(side, use_mode_color)
	return {
		...
		color = function()
			...
			local color = use_mode_color and get_mode_color() or
					get_opposite_color(get_mode_color())
			return {
				fg = color,
			}
		end,
...
ins_left(
	create_mode_based_component('filename', nil, colors.BG)
)
...
ins_right(
	create_mode_based_component('location', nil, colors.BG)
)
